								...
								{ !next ? (
									<motion.div key={"0"} className='w-full h-[calc(100vh-5rem)]'>
										<Box className='bg-white pt-16 w-full h-full rounded relative'>
											...
											<Container maxWidth={"lg"} className='w-full my-0 h-[calc(100%-4rem)] overflow-y-auto'>
													<GradientHead text={ 'Authenticate using phone number.' } className='py-0 text-3xl w-full text-left' />
													<div className='flex flex-col lg:flex-row justify-between lg:mt-12 w-full items-center'>
														<PhoneAuth done={ () => setNext( true ) }  updateData={ setUserObject } />
														<img src="auth.svg" className='h-full w-full lg:w-1/2 lg:h-1/2 origin-center object-contain' alt="" />
													</div>
											</Container>
										</Box>
									</motion.div>
